...
function fn_paging(pageNum){
	...
	location.href="${contextPath}/community/community.do?curPage="+pageNum+"&dt_fr="+dt_fr+"&dt_to="+dt_to+"&search_select="+search_select+"&search_keyword="+search_keyword;
}
...
<body>
     ...
     <input type="hidden" id="dt_fr" value="${condMap.dt_fr}">
     <input type="hidden" id="dt_to" value="${condMap.dt_to}">
     <input type="hidden" id="search_select" value="${condMap.search_select}">
     <input type="hidden" id="search_keyword" value="${condMap.search_keyword}">
     <div class="first_text"><!--상단 제목-->
                ...
                <c:forEach var="viewAll" items="${viewAll}">
                  <c:if test="${viewAll.article_category == '자유게시판'}">
                    ...
                    <tr>
                    	...
                    	<c:if test="${viewAll.comment_cnt != 0 }">
                    		<td id="title_td">
                    			<a href="${contextPath}/community/communityDetail.do?articleNo=${viewAll.articleNo}">
                    				${viewAll.title}&nbsp;<span style="color:tomato;font-size:14px;">[${viewAll.comment_cnt}]</span>
                    			</a>
                    		</td>
                    	</c:if>
                    ...
                    <c:if test="${viewAll.state eq 'DU'}">
                    	<tr>
                    		<td colspan="5" id="title_id" style="text-align:left;color:#676767;">삭제된 게시글입니다.</td>
                    	</tr>
                    </c:if>
                    <c:if test="${viewAll.state eq 'DA'}">
                    	<tr>
                    		<td></td>
                    		<td colspan="4" id="title_id" style="text-align:left;color:#676767;">관리자에 의해 숨겨진 게시글입니다.</td>
                    	</tr>
                    </c:if>
                   </c:if>
                 </c:forEach>
		...
		<div class="noticeWrite_btn_box">
          ...
          <div>
                ...
                <form name="searchForm" id="searchForm"  method="get" action="${contextPath}/community/community.do">
